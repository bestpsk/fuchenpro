  ...
  <view class="detail-container">
    ...
    <view class="audit-section" v-if="canAudit && detailMode !== 'operation'">
      ...
      <view class="audit-btns">
        <u-button v-if="(orderInfo.orderStatus || orderInfo.status) === '0' && checkPermi('business:order:enterpriseAudit')" type="primary" text="企业审核" @click="handleEnterpriseAudit"></u-button>
        <u-button v-if="(orderInfo.orderStatus || orderInfo.status) === '1' && checkPermi('business:order:financeAudit')" type="success" text="财务审核" @click="handleFinanceAudit"></u-button>
        <u-button v-if="(orderInfo.orderStatus || orderInfo.status) === '0' && checkPermi('business:order:cancel')" type="error" plain text="取消订单" @click="handleCancelOrder"></u-button>
      </view>
    </view>
  </view>
</template>
