<template>
  <view class="card-item-container">
    ...
    <u-popup :show="showFilter" mode="top" round="16" @close="toggleFilter">
      <view class="popup-content">
        ...
        <view class="form-item">
          <view class="form-label">卡项分类</view>
    ...
    >
      <view v-if="cardItemList.length > 0" class="card-list">
        <view
        ...
        >
          <view class="card-header">
            <view class="card-item-name">
              ...
              <text class="name-text">{{ item.cardItemName }}</text>
            </view>
          ...
          <view class="card-body">
            <view class="info-row">
              <view class="info-item">
                ...
                <text class="value">{{ item.cardItemCode || '-' }}</text>
              </view>
              <view class="info-item">
            ...
            <view class="info-row">
              <view class="info-item">
                <text class="label">默认数量</text>
                <text class="value">{{ item.defaultQuantity ?? '-' }}</text>
              </view>
              <view class="info-item">
                <text class="label">建议价格</text>
                <text class="value highlight">{{ item.suggestedPrice ? '¥' + item.suggestedPrice : '-' }}</text>
              </view>
            </view>
            <view class="info-row">
              <view class="info-item">
                <text class="label">默认单价</text>
                <text class="value highlight">{{ item.defaultUnitPrice ? '¥' + item.defaultUnitPrice : '-' }}</text>
              </view>
              <view class="info-item">
                <text class="label">状态</text>
                <view v-if="checkPermi('business:cardItem:edit')" class="switch-wrap" @click.stop="toggleStatus(item)">
                  <u-switch
                ...
                <text v-else class="value">{{ item.status === '0' ? '正常' : '停用' }}</text>
              </view>
            </view>
          </view>
          ...
          <view class="card-footer">
            ...
            <view class="action-btns">
              <view v-if="checkPermi('business:cardItem:edit')" class="action-btn edit" @click.stop="goEdit(item)">
                <u-icon name="edit-pen" size="14"></u-icon>
                <text>编辑</text>
              </view>
              <view v-if="checkPermi('business:cardItem:remove')" class="action-btn delete" @click.stop="handleDelete(item)">
    ...
    <view v-if="checkPermi('business:cardItem:add')" class="fab-btn" @click="goAdd">
...
/** 加载卡项列表，支持分页和关键词搜索（搜索时同时匹配卡项名称/编码），isRefresh为true时重置到第一页 */
async function getList(isRefresh = false) {
  ...
  try {
    ...
    if (isRefresh) {
      cardItemList.value = list
    } else {
      cardItemList.value = [...cardItemList.value, ...list]
    }

    if (cardItemList.value.length >= total) {
...
async function toggleStatus(item) {
  ...
  try {
    await updateCardItem({ cardItemId: item.cardItemId, status: newStatus })
    item.status = newStatus
    uni.showToast({ title: newStatus === '0' ? '已启用' : '已停用', icon: 'success' })
  } catch (e) {
...
function goEdit(item) {
  uni.navigateTo({
    url: `/pages/business/cardItem/form?id=${item.cardItemId}&mode=edit`
  })
}
...
function goAdd() {
  uni.navigateTo({
    url: '/pages/business/cardItem/form?mode=add'
  })
}

/** 删除卡项，弹出确认框后调用删除接口，成功后刷新列表 */
function handleDelete(item) {
  uni.showModal({
    title: '提示',
    content: `是否确认删除"${item.cardItemName}"?`,
    success: async (res) => {
      if (res.confirm) {
        try {
          await delCardItem(item.cardItemId)
