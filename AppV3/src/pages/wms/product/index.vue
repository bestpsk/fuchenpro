  ...
  <view class="product-container">
    ...
    <scroll-view scroll-y class="list-scroll" @scrolltolower="loadMore" refresher-enabled :refresher-triggered="refreshing" @refresherrefresh="onPullDownRefresh">
      <view v-if="productList.length > 0" class="card-list">
        <view v-for="item in productList" :key="item.productId" class="product-card" @click="goDetail(item)">
          ...
          <view class="card-body">
            ...
            <view class="info-row">
              ...
              <view class="info-item">
                ...
                <text class="info-value price">¥{{ formatAmount(item.sellingPrice) }}</text>
              </view>
            </view>
          </view>
          <view class="card-actions" v-if="hasActions" @click.stop>
            <view v-if="canEdit" class="action-tag edit" @click.stop="goEdit(item)">编辑</view>
            <view v-if="canDelete" class="action-tag delete" @click.stop="handleDelete(item)">删除</view>
          </view>
        </view>
      </view>
      <u-empty v-else-if="!loading" mode="data" text="暂无货品数据" :marginTop="100"></u-empty>
      <u-loadmore :status="loadStatus" :loading-text="'加载中...'" :loadmore-text="'上拉加载更多'" :nomore-text="'没有更多了'" :marginTop="20" />
    </scroll-view>

    <view v-if="checkPermi('wms:product:add')" class="fab-btn" @click="handleAdd">
...
const queryParams = reactive({ pageNum: 1, pageSize: 10, keyword: '', status: '', category: '' })
...
async function getList(isRefresh = false) {
  ...
  try {
    ...
    loadStatus.value = productList.value.length >= total ? 'nomore' : 'loadmore'
  } catch (e) { console.error('获取货品列表失败:', e); loadStatus.value = 'error' }
...
function resetFilter() { queryParams.status = ''; queryParams.category = '' }
function confirmFilter() { showFilter.value = false; getList(true) }

function goDetail(item) {
  uni.navigateTo({ url: `/pages/wms/product/detail?id=${item.productId}` })
}

function goEdit(item) {
  uni.navigateTo({ url: `/pages/wms/product/form?mode=edit&id=${item.productId}` })
}

function handleAdd() {
  uni.navigateTo({ url: '/pages/wms/product/form?mode=add' })
}

function handleDelete(item) {
  uni.showModal({ title: '提示', content: '确认删除该货品？删除后不可恢复。', success: async (res) => {
    if (res.confirm) {
      try {
        await delProduct(item.productId)
...
.product-container { display: flex; flex-direction: column; height: 100%; overflow: hidden; padding: 0 24rpx;
...
.product-card { background: #fff; border-radius: 16rpx; padding: 28rpx 32rpx; transition: all 0.15s;
