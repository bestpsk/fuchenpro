  ...
  <view class="detail-container">
    <view class="info-card">
      <view class="card-header-row">
        <text class="warehouse-name">{{ info.warehouseName || '-' }}</text>
        <view class="status-badge" :class="'status-' + info.status">{{ info.status === '0' ? '正常' : '停用' }}</view>
      </view>
      <view class="info-body">
        <view class="info-row">
          <text class="info-label">仓库编码</text>
          <text class="info-value">{{ info.warehouseCode || '-' }}</text>
        </view>
        <view class="info-row">
...
import { getWarehouse, delWarehouse } from '@/api/wms/warehouse'
...
function goEdit() {
  uni.navigateTo({ url: `/pages/wms/warehouse/form?mode=edit&id=${warehouseId.value}` })
}
...
function handleDelete() {
  uni.showModal({
    ...
    success: async (res) => {
      if (res.confirm) {
        try {
          await delWarehouse(warehouseId.value)
...
.warehouse-name { font-size: 32rpx; font-weight: 600; color: #1D2129; flex: 1; margin-right: 16rpx; }
