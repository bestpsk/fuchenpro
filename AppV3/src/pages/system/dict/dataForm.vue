<template>
  <view class="form-container">
    <view class="form-section">
      <view class="form-field">
        <view class="field-input-box readonly">
          ...
          <input class="field-input" :value="form.dictType" placeholder="字典类型" placeholder-class="field-placeholder" disabled :disabledColor="'#F7F8FA'" />
        </view>
      </view>

      <view class="form-field">
        <view class="field-input-box">
          <u-icon name="tag" size="18" color="#86909C"></u-icon>
          <input class="field-input" type="text" v-model="form.dictLabel" placeholder="* 数据标签" placeholder-class="field-placeholder" />
        </view>
      </view>

      <view class="form-field">
        <view class="field-input-box">
          <u-icon name="key" size="18" color="#86909C"></u-icon>
          <input class="field-input" type="text" v-model="form.dictValue" placeholder="* 数据键值" placeholder-class="field-placeholder" />
        </view>
      </view>

      <view class="form-field">
        <view class="field-input-box">
          <u-icon name="star" size="18" color="#86909C"></u-icon>
          <input class="field-input" type="text" v-model="form.cssClass" placeholder="样式属性" placeholder-class="field-placeholder" />
        </view>
      </view>
      ...
      <view class="form-field">
        <view class="field-input-box">
          ...
          <input class="field-input" type="number" v-model="form.dictSort" placeholder="* 显示排序" placeholder-class="field-placeholder" />
        </view>
      </view>

      <view class="form-field" @click="showListClassPicker = true">
        <view class="field-input-box">
          <u-icon name="palette" size="18" color="#86909C"></u-icon>
          <input class="field-input" :value="listClassName" placeholder="回显样式" placeholder-class="field-placeholder" disabled :disabledColor="'#fff'" />
    ...
    <u-picker
      :show="showListClassPicker"
      :columns="[listClassOptions]"
      keyName="label"
      title="选择回显样式"
      @confirm="onListClassConfirm"
      @cancel="showListClassPicker = false"
      @close="showListClassPicker = false"
    ></u-picker>
...
const listClassName = computed(() => {
  const item = listClassOptions.value.find(o => o.value === form.listClass)
  return item ? item.label : ''
})

function onListClassConfirm(e) {
  const item = e.value[0]
  form.listClass = item.value
...
async function loadDetail() {
  ...
  try {
    ...
    const res = await getData(dictCode.value)
    const data = res.data || {}
    Object.assign(form, {
...
async function submitForm() {
  if (!form.dictLabel) { uni.showToast({ title: '请输入数据标签', icon: 'none' }); return }
  if (!form.dictValue) { uni.showToast({ title: '请输入数据键值', icon: 'none' }); return }
  if (form.dictSort === '' || form.dictSort === null || form.dictSort === undefined) {
...
function goBack() {
  ...
  else uni.redirectTo({ url: `/pages/system/dict/data?dictType=${dictType.value}` })
}
...
onMounted(() => {
  ...
  dictCode.value = options.id ? parseInt(options.id) : null
  dictType.value = options.dictType || ''
  form.dictType = dictType.value

  if (mode.value === 'add') {
    uni.setNavigationBarTitle({ title: '新增字典数据' })
  } else if (mode.value === 'edit') {
    uni.setNavigationBarTitle({ title: '编辑字典数据' })
    loadDetail()
  }
})
</script>
