  ...
  <view class="dict-container">
    <view class="search-section">
      <view class="search-box">
        ...
        <view v-if="checkPermi('system:dict:remove')" class="cache-btn" @click="handleRefreshCache">
    ...
    <u-popup :show="showFilter" mode="top" round="16" @close="toggleFilter">
      <view class="popup-content">
        ...
        <view class="form-item">
          <view class="form-label">字典状态</view>
        ...
        <view class="form-item">
          ...
          <view class="date-range" @click="showDatePicker = true">
            <text :class="{ 'date-placeholder': !dateRangeText }">{{ dateRangeText || '请选择时间范围' }}</text>
    ...
    >
      <view v-if="dictList.length > 0" class="card-list">
        <view
        ...
        >
          <view class="card-header">
            <view class="dict-name">
              <u-icon name="list" size="18" color="#3D6DF7"></u-icon>
              <text class="name-text">{{ item.dictName }}</text>
            </view>
          ...
          <view class="card-body">
            <view class="info-row">
              <view class="info-item">
                ...
                <text class="value link-text" @click.stop="goDictData(item)">{{ item.dictType }}</text>
              </view>
              <view class="info-item">
          ...
          <view class="card-footer">
            <view class="time-text">编号: {{ item.dictId }}</view>
...
async function getList(isRefresh = false) {
  ...
  try {
    ...
    dictList.value = isRefresh ? list : [...dictList.value, ...list]
    loadStatus.value = dictList.value.length >= total ? 'nomore' : 'loadmore'
  } catch (e) {
...
function onDateConfirm(e) {
  ...
  if (dateRange.value.length === 0 || dateRange.value.length === 2) {
...
function goDictData(item) {
  uni.navigateTo({ url: `/pages/system/dict/data?dictType=${item.dictType}` })
}

function goEdit(item) {
  uni.navigateTo({ url: `/pages/system/dict/form?mode=edit&id=${item.dictId}` })
}

function goAdd() {
  uni.navigateTo({ url: '/pages/system/dict/form?mode=add' })
}

function handleDelete(item) {
  uni.showModal({
    title: '提示',
    content: `是否确认删除字典类型"${item.dictName}"?`,
    success: async (res) => {
      if (res.confirm) {
        try {
...
.dict-container { display: flex; flex-direction: column; height: 100%; overflow: hidden; padding: 0 24rpx;
...
.cache-btn {
  flex-shrink: 0; display: flex; align-items: center; justify-content: center;
  width: 56rpx; height: 56rpx; background: rgba(255, 255, 255, 0.3); border-radius: 28rpx;
  &:active { opacity: 0.7; }
}
...
.info-item {
  ...
  .link-text { color: #3D6DF7; text-decoration: underline; }
}
