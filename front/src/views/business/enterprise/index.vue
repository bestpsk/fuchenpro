  ...
  <div class="app-container">
    ...
    <el-table v-loading="loading" :data="enterpriseList" @selection-change="handleSelectionChange" @row-click="handleRowClick" style="width: 100%" class="pointer">
      ...
      <el-table-column label="状态" align="center" prop="status" min-width="60">
        <template #default="scope">
          <el-switch v-model="scope.row.status" active-value="0" inactive-value="1" @change="(val) => handleStatusChange(scope.row, val)" v-hasPermi="['business:enterprise:edit']" />
        </template>
      </el-table-column>
...
const { queryParams, form, rules, planForm, planRules } = toRefs(data)
