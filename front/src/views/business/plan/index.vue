<template>
  <div class="app-container">
    ...
    <el-table v-loading="loading" :data="enterpriseList">
      ...
      <el-table-column label="开始合作" align="center" prop="cooperationStartDate" />
      <el-table-column label="结束合作" align="center" prop="cooperationEndDate" />
      <el-table-column label="方案数量" align="center">
      ...
      <el-table-column label="操作" align="center">
        <template #default="scope">
          <el-button link type="primary" icon="View" @click="handleViewPlans(scope.row)">查看方案</el-button>
    ...
    <el-dialog :title="planTitle" v-model="planOpen" width="1200px" append-to-body>
      <el-form ref="planRef" :model="planForm" :rules="planRules" label-width="100px">
        <el-row>
          ...
          <el-col :span="12">
            <el-form-item label="回款比例(%)" prop="commissionRate">
...
function getList() {
  loading.value = true
  listEnterprise(queryParams.value).then(res => {
...
function handleEnterpriseStatusChange(row, val) {
  ...
  proxy.$refs["planRef"].validate(valid => {
    if (valid) {
      if (planForm.value.planId != undefined) {
        updatePlan(planForm.value).then(() => { proxy.$modal.msgSuccess("修改成功"); planOpen.value = false; handleViewPlans(currentEnterprise.value) })
      } else {
        addPlan(planForm.value).then(() => { proxy.$modal.msgSuccess("新增成功"); planOpen.value = false; handleViewPlans(currentEnterprise.value) })
      }
    }
  })
}
